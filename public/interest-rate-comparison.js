(() => {
  ...
  const render = () => {
    ...
    rows.innerHTML = filtered.length ? filtered.map((lender) => {
      ...
      return `<tr><td><div class="rate-lender"><img src="/lender-logos/${escapeHtml(logo)}" alt="${escapeHtml(lender.name)} logo" onerror="this.onerror=null;this.src='/lender-logos/unavailable.svg';this.alt='Logo unavailable'"><span>${escapeHtml(lender.name)}</span></div></td><td>${escapeHtml(rate)}</td><td>Not published</td><td>${escapeHtml(detail(lender, "fee"))}</td><td>${escapeHtml(loan)}</td><td>${escapeHtml(detail(lender, "tenure"))}</td><td>Illustrative; lender approval and pricing depend on your profile.</td><td><a class="rate-action" href="/apply?lender=${encodeURIComponent(lender.slug)}">View details</a></td></tr>`;
