(() => {
  ...
  section.innerHTML = `<div class="cw-home-journey__heading"><p>EDUCATION LOAN GUIDANCE</p><h2 id="journey-title">A clearer loan journey, step by step.</h2><span>From your first questions to lender review, each step helps you make a more informed choice.</span></div><ol>${steps.map(([number, heading, copy, icon]) => `<li><article tabindex="0"><span class="cw-home-journey__number">${number}</span><i aria-hidden="true">${icon}</i><h3>${heading}</h3><p>${copy}</p></article></li>`).join("")}</ol><a class="cw-home-journey__cta" href="/eligibility">Check your eligibility <span aria-hidden="true">→</span></a>`;
})();
