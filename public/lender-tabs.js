...
(() => {
  ...
  const classify = async () => { try { const response = await fetch('/api/lenders'); if (!response.ok) throw new Error(); const lenders = await response.json(); const types = new Map(lenders.map((lender) => [lender.name, String(lender.lenderType).toLowerCase()])); root.querySelectorAll('tbody tr').forEach((row) => { const name = row.querySelector('.lc-lender b')?.textContent?.trim() || ''; row.dataset.lenderType = types.get(name) || ''; }); select(category); } catch { /* Table remains usable if filtering data fails. */ } };
