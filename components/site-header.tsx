...
export function SiteHeader() {
  ...
  const close = useCallback(() => { cancelClose(); setOpen(null); setMobileOpen(false); }, [cancelClose]);
  ...
  useEffect(() => {
    ...
    const onPointerDown = (event: PointerEvent) => { if (!(event.target instanceof Element) || !event.target.closest("[data-cw-header]")) close(); };
  ...
  const menu = (name: string, links: readonly (readonly [string, string])[], mobile = false) => {
    const menuId = id(name, mobile);
    const isOpen = open === menuId;
    return <div
      className="cw-group"
      key={menuId}
      onMouseEnter={() => { if (!mobile) { cancelClose(); setOpen(menuId); } }}
      onMouseLeave={() => { if (!mobile) scheduleClose(); }}
      onFocusCapture={() => { if (!mobile) { cancelClose(); setOpen(menuId); } }}
      onBlurCapture={(event) => {
        if (mobile) return;
    ...
    >
      <button className="cw-toggle" type="button" aria-expanded={isOpen} aria-controls={menuId} onClick={() => { cancelClose(); setOpen((current) => current === menuId ? null : menuId); }}>
        {name}<svg className="cw-chevron" viewBox="0 0 16 16" fill="none" aria-hidden="true"><path d="m3 6 5 5 5-5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>
      </button>
  ...
  return <header className="cw-header" data-cw-header="true">
    <nav className="cw-nav" aria-label="Main navigation">
      <a className="cw-logo" href="index.html" aria-label="Credwisory home"><i aria-hidden="true" /><span>Credwisory<small>SOLUTIONS LLP</small></span></a>
      <div className="cw-desktop">{groups.map(([name, links]) => menu(name, links))}</div>
      <a className="cw-cta" href="eligibility.html">Check eligibility</a>
      <button className="cw-burger" type="button" aria-label="Open navigation" aria-expanded={mobileOpen} aria-controls="cw-mobile" onClick={() => { setMobileOpen(!mobileOpen); setOpen(null); }}><span /><span /><span /></button>
      <div className={`cw-mobile${mobileOpen ? " open" : ""}`} id="cw-mobile">{groups.map(([name, links]) => menu(name, links, true))}</div>
    </nav>
  </header>;
}
