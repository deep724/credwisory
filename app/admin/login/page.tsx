...
export default function LoginPage() {
  ...
  async function login(form: FormData) { setBusy(true); setError(""); try { const response = await fetch("/api/admin/auth/login", { method: "POST", credentials: "same-origin", headers: { "content-type": "application/json" }, body: JSON.stringify({ email: form.get("email"), password: form.get("password") }) }); const data = await response.json().catch(() => null); if (response.ok) { router.replace(data?.passwordChangeRequired ? "/admin/change-password" : "/admin"); router.refresh(); } else setError(typeof data?.error === "string" ? data.error : "Unable to sign in. Please try again."); } catch { setError("Sign-in is temporarily unavailable. Please retry."); } finally { setBusy(false); } }
